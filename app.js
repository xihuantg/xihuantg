...
function paidProgramDetail(item) {
  return state.member
    ...
    `
    : `<p class="locked-line">进阶版查看考研升学、适合人群、大学四年学习建议和替代院校说明。</p>`;
}
...
function renderAssessmentResult() {
  ...
  if (!result) {
    target.innerHTML = `
      <div class="assessment-empty">
        ...
        <p>测评会结合当前选科、位次和可报专业组，给出适合方向、学习建议和院校专业组匹配提示。</p>
      </div>
    `;
  ...
  target.innerHTML = `
    <div class="assessment-profile-card">
      ...
      <p><strong>需要避开的坑：</strong>${state.member ? result.primary.avoid : "进阶版查看不适合方向和完整风险解释。"}</p>
      <p><strong>大学怎么学：</strong>${state.member ? result.primary.studyAdvice : "进阶版查看大学四年学习建议。"}</p>
    </div>
  ...
  grid.innerHTML = visible.map((item, index) => {
    ...
    return `
      <article class="rec-card ${locked ? "locked" : ""}">
        ...
        <div class="stats">
          <span><strong>${locked ? "--" : item.chance + "%"}</strong>录取机会参考</span>
        ...
        <p><strong>学校环境公开评价摘要：</strong>${state.member ? item.environment.replace("公开评价摘要：", "") : "进阶版查看校园环境、宿舍、食堂、交通等公开评价摘要。"}</p>
        ...
        <div class="action-row">
          ...
          <button type="button" data-add="${item.id}">加入候选清单</button>
...
function renderVolunteers() {
  ...
  if (!state.volunteers.length) {
    list.innerHTML = `<p class="muted">还没有添加候选志愿。可在候选专业组或院校专业组样板库中选择“加入候选清单”。</p>`;
  } else {
...
function openSchoolDetail(id, returnTab = state.mobileTab || "recommend") {
  ...
  $("#schoolCareerText").innerHTML = `
    ...
    <p><strong>考研/升学：</strong>${state.member ? item.postgraduatePath : "进阶版查看考研方向、升学依赖度和职业路径解释。"}</p>
  `;
  $("#schoolFitText").innerHTML = `
    ...
    <p><strong>怎么学：</strong>${state.member ? item.studyPlan : "进阶版查看大学四年学习建议。"}</p>
  `;
  ...
  $("#schoolEnvironmentText").innerHTML = state.member
    ...
    : `<p>免费版展示基础信息；进阶版查看完整校园环境、宿舍、食堂、交通等公开评价摘要。</p><ul><li>校园氛围、宿舍食堂、交通通勤会影响四年体验。</li><li>同一学校不同校区差异较大，正式填报前需要核验校区。</li></ul>`;
  $("#schoolSourceText").innerHTML = state.member
    ? `${item.source}<br>${item.officialNote || "最终以考试院和高校官方信息为准。"}`
    : "完整来源和风险说明为进阶分析权益；正式上线前所有摘要需替换为真实可核验来源。";
...
function renderPricing() {
  $("#pricingGrid").innerHTML = pricing.map((plan) => `
    <article class="price-card ${plan.featured ? "featured" : ""}">
      <span class="badge">${plan.featured ? "建议" : "权益"}</span>
      <h3>${plan.name}</h3>
      <div class="price">${plan.price}<small> / ${plan.period || "本届高考季"}</small></div>
      <p>${plan.desc}</p>
      <p>${plan.perks.join(" · ")}</p>
      <button class="${plan.featured ? "primary-btn" : "secondary-btn"}" data-plan="${plan.name}" type="button">${plan.button || "查看" + plan.name}</button>
    </article>
  `).join("");
}
...
function renderReport() {
  ...
  $("#reportScope").textContent = state.member ? `${eligible.length}个样板可报专业组；当前批次最多${rule.maxVolunteers}个${rule.unit}` : `查看基础版后可按${rule.maxVolunteers}个${rule.unit}规则整理候选清单`;
  $("#reportGradient").textContent = `冲 ${counts["冲"] || 0} · 稳 ${counts["稳"] || 0} · 保 ${counts["保"] || 0}`;
  $("#reportStatus").textContent = state.member ? "进阶分析已开启" : "基础核验";
...
function updateProfileSummary() {
  ...
  $("#memberBadge").textContent = state.member ? "进阶分析" : "免费预览";
...
function addVolunteer(id) {
  ...
  if (state.volunteers.some((program) => program.id === id)) {
    showToast("该专业组已经在候选清单中");
    return;
  }
  if (!state.member && state.volunteers.length >= 3) {
    showToast("免费版最多添加3个候选，进阶版可按当前批次规则整理完整候选清单");
    return;
  }
  state.volunteers.push(item);
  renderVolunteers();
  showToast(`${item.school} ${item.group} 已加入候选清单`);
}
...
function exportCsv() {
  if (!state.member) {
    showToast("导出候选清单是进阶版权益，请先查看进阶分析");
    return;
  }
...
function initEvents() {
  ...
  $("#mobileLoginBtn").addEventListener("click", () => showToast("登录可接入手机号验证码和版本订单"));
