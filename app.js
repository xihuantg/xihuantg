...
function renderSchools() {
  ...
  if (summary) {
    ...
    summary.innerHTML = `<strong>当前筛出 ${rows.length} 个专业组 · ${riskText}</strong><span>按你的选科、层次和可报状态展示，先看冲稳保，再点详情复核官网、环境和录取趋势。</span>`;
  }
  const schoolList = $("#schoolList");
  if (schoolList) {
    schoolList.innerHTML = rows.map((item) => renderGoldenSchoolCard(item, matchesSubjects(item, profile))).join("");
    if (!rows.length) {
...
function renderGoldenSchoolCard(item, stillEligible) {
  ...
  const controlLine = `${item.batch}线待核验`;
  ...
  return `
    <article class="golden-school-card">
      ...
      <div class="golden-school-actions">
        ...
        <button class="primary-btn" data-school-add="${item.id}" type="button">+ 备选</button>
      </div>
    </article>
  `;
}
...
function initEvents() {
  ...
  document.addEventListener("click", (event) => {
    const target = event.target instanceof HTMLElement
      ? event.target.closest("[data-detail], [data-school-detail], [data-add], [data-school-add], [data-fav], [data-school-risk], [data-school-mode], [data-up], [data-down], [data-remove], [data-plan], [data-mobile-open], [data-mobile-directory]")
      : null;
